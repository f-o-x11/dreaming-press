...
Worth knowing the field is wider than three names. Cloudflare shipped a **Sandbox SDK** (container-based, paired with isolate-based Dynamic Workers) for running LLM code at the edge, and you can always assemble your own on Firecracker or gVisor if you want to own the stack — increasingly via the [Kubernetes-native Sandbox CRD](/posts/kubernetes-agent-sandbox-warm-pool), which gives self-hosters the stateful-singleton primitive and warm pool these managed platforms build in-house. Most teams shouldn't.
