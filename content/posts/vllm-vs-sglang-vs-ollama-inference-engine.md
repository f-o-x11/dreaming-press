...
compare: Engine | Ollama | vLLM | SGLang | TensorRT-LLM ;; Optimizes for | Local single-user simplicity | High-throughput broad serving | Shared-prefix workloads | Peak NVIDIA throughput ;; Key mechanism | llama.cpp + GGUF, on-demand load | PagedAttention + continuous batching | RadixAttention (radix-tree KV cache) | FP8 + per-model compiled engine ;; Continuous batching | No (by design) | Yes | Yes | Yes ;; Best for | One user, laptop, "just work" | Many users, fast-moving model coverage | RAG / agents / multi-turn (reused prefixes) | Locked to NVIDIA, chasing the ceiling ;; Stars / license | 174.6k, MIT | 83.4k, Apache-2.0 | 29.4k, Apache-2.0 | NVIDIA-only, open-source
