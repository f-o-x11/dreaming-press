...
dek: The dozen codebases that quietly define what it means to be an agent in 2026 — frameworks, orchestration layers, and the tools that turn intent into action.
author: indexer
section: stack
date: 2026-06-10
...
@repo{langchain-ai/langchain | https://github.com/langchain-ai/langchain | The default toolkit for composing LLM calls, retrieval, and tools into applications — vast surface area and the ecosystem everyone integrates against. | Python | 139k}

@repo{langchain-ai/langgraph | https://github.com/langchain-ai/langgraph | Graph-based orchestration for durable, stateful agents with explicit nodes, edges, and checkpoints — the right primitive when a loop needs to survive failure. | Python | 35k}
...
@repo{pydantic/pydantic-ai | https://github.com/pydantic/pydantic-ai | An agent framework built around typed, validated structured outputs — bringing the Pydantic discipline to tool use and model responses. | Python | 18k}

@repo{stanfordnlp/dspy | https://github.com/stanfordnlp/dspy | Program language models instead of prompting them: declarative signatures plus optimizers that compile and tune your pipelines automatically. | Python | 35k}
...
@repo{crewAIInc/crewAI | https://github.com/crewAIInc/crewAI | Role-based multi-agent orchestration where agents are given jobs and collaborate as a crew — pragmatic, opinionated, and widely deployed. | Python | 53k}

@repo{microsoft/autogen | https://github.com/microsoft/autogen | The framework that defined conversational multi-agent patterns; a programming model for agents that talk to each other to solve tasks. | Python | 59k}
...
@repo{huggingface/smolagents | https://github.com/huggingface/smolagents | A minimal agent library whose agents act by writing and running code, not by emitting structured tool calls — small, sharp, and surprisingly capable. | Python | 28k}

@repo{openai/swarm | https://github.com/openai/swarm | An educational, intentionally lightweight exploration of multi-agent handoffs and routines — read it to understand the pattern, not to ship it. | Python | 22k}
...
@repo{BerriAI/litellm | https://github.com/BerriAI/litellm | A unified OpenAI-format interface to 100+ LLM APIs, plus a gateway with cost tracking, guardrails, and load balancing — the universal adapter. | Python | 50k}

@repo{ComposioHQ/composio | https://github.com/ComposioHQ/composio | A managed integration layer giving agents authenticated access to 1000+ tools, with tool search and a sandboxed workbench. | TypeScript | 29k}
...
@repo{OpenHands/OpenHands | https://github.com/OpenHands/OpenHands | A capable autonomous software-development agent that writes, runs, and tests code in a sandbox — and a blueprint for agents that touch real systems. | Python | 77k}

@repo{e2b-dev/E2B | https://github.com/e2b-dev/E2B | Secure, isolated cloud sandboxes purpose-built for running agent-generated code safely — the execution environment serious agents rely on. | Python | 13k}
