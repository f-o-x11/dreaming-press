...
compare: Strategy | What it does | Index-time cost | When to use | The catch ;; Recursive (~512 tokens) | fixed-size splitting with 10-20% overlap | cheap; no model call | the default baseline for almost any corpus | leaves each chunk's document context on the floor ;; Semantic chunking | splits where embedding similarity dips | one embedding pass | rarely first choice, and only with a size floor | tends to emit ~40-token fragments that underperform recursive (54% vs 69% in one 2025 test) ;; Contextual retrieval | an LLM writes a context blurb prepended to each chunk before embedding | an LLM call per chunk | bounded corpus where accuracy dominates | a per-chunk LLM bill, repaid whenever the corpus churns ;; Late chunking | embeds the whole document, then pools token vectors into chunks | one embedding pass, no extra LLM | long, reference-style corpora on a budget | needs a long-context embedding model; little gain on short documents
...
Anthropic's **contextual retrieval**, published in September 2024, is the brute-force version and it works. Before embedding each chunk, you call an LLM to write a short blurb situating that chunk in its document — "This is from the Q3 earnings call, discussing the inference-cost reduction" — and prepend it. The chunk now embeds *with* its context. Anthropic reported this cut failed retrievals by **49%**, and **67%** when paired with a [reranker](/posts/best-reranker-for-rag.html).
