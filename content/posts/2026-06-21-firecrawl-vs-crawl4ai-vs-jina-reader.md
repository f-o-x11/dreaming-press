...
summary: Firecrawl, Crawl4AI, and Jina Reader all turn a webpage into clean LLM-ready markdown — they don't actually compete on markdown quality. ;; They sit on three rungs: Jina Reader is a fetch-and-clean primitive (a URL prefix), Crawl4AI is a self-hosted extraction framework (you pay the rendering cost, zero per-page fee), Firecrawl is a managed extraction platform (a metered API with schema-driven structured extraction). ;; Pick by who should pay the rendering cost and where the extraction logic lives, not by star count: a high-volume RAG ingest favors self-hosted Crawl4AI; a one-off real-time read favors the Jina prefix.
faq: Why convert webpages to markdown for an LLM instead of feeding it raw HTML? | Raw HTML is ~80–90% nav, CSS, and script — the model spends tokens and attention on noise. Converting an article to clean markdown commonly cuts the token count by 80% or more, a direct cost line at agent scale, and it survives JS-rendered pages where a naive fetch returns an empty shell. ;; Firecrawl vs Crawl4AI — which should I use? | Crawl4AI is a free, self-hosted Python library: you run the browser, pay nothing per page, and keep full data sovereignty — best for a high-volume RAG ingest pipeline. Firecrawl is a hosted, credit-metered API with schema-driven structured extraction — best when you want extraction at scale without operating a browser fleet yourself. ;; What's the simplest way to read one URL as markdown in an agent? | Jina Reader — prepend https://r.jina.ai/ to any URL and get markdown back, no SDK or schema. It's a fetch-and-clean primitive ideal for a single mid-task read, and it deliberately stops at clean text rather than structured extraction.
...
compare: Dimension | Jina Reader | Crawl4AI | Firecrawl ;; What it is | Fetch-and-clean primitive (URL prefix) | Self-hosted extraction framework | Managed extraction platform ;; Interface | Prepend r.jina.ai/ to a URL | `pip install` library / Docker FastAPI | Hosted API (/scrape, /crawl, /map, /search) ;; Where rendering runs | Hosted endpoint (or self-host the OSS image) | Your CPU/RAM (Playwright) | Their browser fleet ;; Per-page cost | Hosted, low-friction | Zero at the margin | Credit-metered ;; Structured extraction | No — stops at clean text | Yes (CSS/XPath, BM25, LLM) | Yes (schema-driven, LLM mid-crawl) ;; Language | TypeScript | Python | TypeScript ;; Stars | ~11k | ~69k | ~136k ;; Reach for it when | One URL, mid-task, zero setup | High-volume standing ingest pipeline | Structured extraction at scale, no browser ops
---
