...
compare: Dimension | RAG | Long Context ;; How it works | Retrieve top-k chunks into the prompt | Pour the whole corpus into the window ;; Failure mode | Loud — a missing chunk shows up as a dropped citation or "I don't know" | Silent — the fact is in the window and the model reads past it ;; Effective capacity | Bounded by retriever quality, not window size | Effective window far below advertised (NoLiMa puts GPT-4o near ~8K vs 128K) ;; Cost per query | Low — only retrieved chunks enter the prompt | High — token cost scales with how much you stuff ;; Best for | Corpora larger than any window, per-user access control, freshness, citations | Bounded corpus that fits, synthesis across the whole set, reasoning about what's absent ;; Main risk | A bad retrieval step starves the model | Lost-in-the-middle and degraded reasoning as the window fills
