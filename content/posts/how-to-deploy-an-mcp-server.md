...
compare: "Platform | Shape | State model | Auth handled for you | Best fit ;; Vercel | Serverless (Fluid Compute, `mcp-handler`) | Stateless by design — add Redis/Upstash for sessions; Node.js runtime only (not Edge) | Built-in OAuth | A stateless Streamable HTTP server you want to autoscale ;; Cloudflare | Edge actors (`McpAgent` + Durable Objects) | Stateful — one Durable Object per session, SQLite state, survives hibernation | OAuth Provider library (same Worker is auth + resource server) | A stateful server that still scales; per-session continuity ;; FastMCP Cloud | Push-a-repo managed (deploys from a GitHub repo) | Managed; live URL in ~a minute, auto-detects deps, PR preview deploys | Handles the OAuth wiring for you | Python FastMCP servers you want hosted fast ;; Smithery | Managed auto-scaling infra + public registry | Managed via `smithery.yaml`; doubles as a 7,000-server discovery registry | Managed | Hosting plus discoverability ;; Fly.io | Persistent Machine (wraps an existing stdio server) | Stateful/persistent; auto-stop/auto-suspend so an idle server costs ~nothing | Bearer-token auth on by default | Wrapping an existing stdio server cheaply"
art:
  archetype: grid
  mood: cold
