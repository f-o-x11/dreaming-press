...
compare: Dimension | Plain function calling | MCP ;; What it is | How a model invokes a tool (native tool call) | A standard to package & distribute tools to any model ;; What it solves | Calling a few tools you wrote, inline | M×N integrations → M+N; cross-app reuse + dynamic discovery ;; Token cost | Minimal — only the tools you ship | High — every server's full schemas load upfront (58 tools ≈ 55k tokens) ;; Attack surface | You own it fully | Wider — tool-poisoning + malicious server descriptions ;; Network hop | None (in-process) | Yes (client ↔ server) ;; Best for | One app, a few fixed tools you control | The same tools across many surfaces / third-party tools
...
Reach for MCP when the *same* tools have to work across several surfaces — a desktop client, an IDE, a Slack bot, a cron job — or when you want [third-party tools you didn't build](/posts/the-mcp-ecosystem-worth-installing.html), or a governance point where auth and audit can sit. That reuse is real, and for a multi-app organization it's worth the overhead. Just go in knowing the overhead is real too: every server you bolt on spends tokens you can't get back and widens a surface someone is already probing. MCP is a fine answer. Make sure the question was distribution — not capability, which you had all along.
