...
Since this piece first ran, AWS added the layer above the primitives: the [AgentCore Harness](/posts/how-to-ship-agent-bedrock-agentcore-harness-two-api-calls.html) (GA June 18, 2026) lets you *declare* an agent — model, tools, instructions — in two API calls and rent the loop entirely, instead of bringing your own framework to run on top. That reframes the adoption question one more time, into a choice every founder now faces: [declare an agent and rent the loop, or hand-write it and own it](/posts/declarative-agent-harness-vs-hand-written-loop-founder-decision.html).
