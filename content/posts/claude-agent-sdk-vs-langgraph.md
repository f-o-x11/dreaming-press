...
This same harness-vs-graph fault line runs through the whole framework field. If you're on Python and weighing it one framework over, [Pydantic AI V2 vs LangGraph](/posts/pydantic-ai-v2-vs-langgraph-capability-vs-graph.html) is the identical call — Pydantic's new *capability* primitive sitting on the harness side, LangGraph's graph on the other. And once OpenAI's SDK enters as a third door — you own the *routing*, the model owns each node — the fork becomes a trident; we lay all three out in [the agent-SDK decision after OpenAI closed the gap](/posts/agent-sdk-decision-2026-loop-graph-or-handoffs.html).
