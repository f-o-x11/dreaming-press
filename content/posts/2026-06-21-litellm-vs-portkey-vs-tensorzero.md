...
summary: An LLM gateway sits between your agent and the model providers, and the three most-installed open-source options have stopped being the same kind of thing — they now disagree about how much of your stack the gateway should absorb. ;; The clearest signal is the language each chose: LiteLLM (Python) wants to be a library inside your app; Portkey's gateway (TypeScript) wants to be a language-agnostic reliability layer at the edge; TensorZero (Rust) wants to be a standalone data plane where observability, evals, and optimization converge. ;; Choose by where you want that seam to sit, not by counting supported providers — they all support more than you'll use. LiteLLM for least-friction Python adoption; Portkey when non-Python clients need the same routing; TensorZero when you actually want the experimentation/feedback machinery in one place.
faq: What does an LLM gateway actually do? | It is the single place every model call passes through, so it can normalize provider-specific request formats behind one API, add retries and fallbacks across providers, balance load, enforce per-key budgets, and log spend and outcomes. Without one, that logic gets scattered across every call site in your codebase. ;; Is LiteLLM or Portkey better for a production agent? | They optimize for different things. LiteLLM is frictionless for Python apps and runs as a library or proxy, but at high volume in your hot path you eventually contend with proxy overhead and the GIL. Portkey's gateway is built to deploy at the edge as a separate service, giving browser, Go, and Python clients identical routing and guardrails — at the cost of operating one more moving part. ;; When is TensorZero worth the extra weight? | When you want the gateway to be your LLMOps data plane, not just a router. It writes every inference and feedback signal to your own database and folds observability, A/B experiments, evaluations, and optimization into one Rust service. That only pays off if you'll actually use the experimentation machinery; if you just need API normalization, it is a lot of platform to stand up.
art:
