...
compare: Approach | Vector RAG | Microsoft GraphRAG | LightRAG | LazyGraphRAG ;; Best at | Local lookups (answer lives in a few chunks) | Global, sensemaking questions | Global, with a lighter graph | Global at vector-RAG cost ;; Index cost | Baseline | ~1,000x vector RAG (LLM reads the whole corpus) | Lighter — drops community detection | ~Vector-RAG cost (LLM work deferred to query time) ;; Global-query cost | Can't really answer them | Consults pre-written community summaries | Graph traversal | 700x+ cheaper than classic GraphRAG ;; Graph database needed | No (vector store) | No (files / embedded store) | No | No ;; Reach for it when | Most teams — local Q&A | Domain is relational; global Qs proven in logs | Global Qs on a budget | Global Qs at scale, cheapest path
