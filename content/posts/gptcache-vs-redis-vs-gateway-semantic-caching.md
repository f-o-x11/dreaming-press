...
For everything else — FAQs, docs assistants, repetitive support queries, the long tail of "how do I…" questions that thousands of users ask in slightly different words — semantic caching is one of the highest-leverage cost cuts available, and it composes cleanly with [prompt caching](/posts/prompt-caching-for-ai-agents) rather than competing with it. Use prompt caching to make the calls you keep cheaper, and semantic caching to delete the calls you never needed to make twice. The skill isn't installing either one. It's knowing which questions are safe to answer from memory.
