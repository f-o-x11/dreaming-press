...
compare: Dimension | Unsloth | Axolotl | torchtune ;; The bet (bottleneck) | Single-GPU speed & memory | Configuration surface area | Hackability / native distributed ;; Best for | Solo dev, one 4090 or A100 | Teams running many experiments | Rewriting the training loop, clean scaling ;; Interface | Python + Colab notebooks | One YAML config | Forkable PyTorch recipes ;; Multi-GPU / multi-node | Limited | Strong (DDP/FSDP/DeepSpeed) | Native FSDP2 ;; License | Apache-2.0 | Apache-2.0 | BSD-3-Clause ;; 2026 status | Active | Active (v0.17, Jun 2026) | Maintenance mode (wound down 2025)
faq: What is the fastest LLM fine-tuning framework? | On a single GPU, Unsloth — its hand-written Triton kernels claim up to 2x faster training with up to 70% less VRAM. But "fastest" only decides it if single-device throughput is your actual bottleneck; many-experiment teams and custom training loops are better served by Axolotl or torchtune despite not winning raw speed. ;; Is torchtune still maintained in 2026? | No. The torchtune repo carries a notice that development wound down in 2025; it now receives only critical fixes while the team builds a successor. The recipes still run and still teach, but betting a team's next year of training on it carries roadmap risk. ;; Should I use Unsloth or Axolotl? | Use Unsloth if your wall is "it won't fit or it's too slow on my one GPU." Use Axolotl if your wall is "I'm running many experiments across multiple GPUs and need them reproducible from a config in git." They optimize different bottlenecks.
