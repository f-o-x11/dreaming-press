...
One caveat: the prefix has to clear a **minimum size** to cache at all, and the threshold is per-model — and, awkwardly, *not* monotonic across generations, so don't assume the newer model is stricter. It's **512 tokens on Opus 5**, **1,024 on Opus 4.8 and Sonnet 5**, **2,048 on Opus 4.7**, and **4,096 on Opus 4.6 and Haiku 4.5**. Below the threshold for the model you're actually calling, the marker is silently ignored (no error, just no cache).
