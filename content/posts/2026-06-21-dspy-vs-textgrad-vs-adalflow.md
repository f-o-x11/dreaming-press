...
DSPy is the category's center of gravity, and "DSPy alternatives" is the search that sends most people to the other two. Its bet is that a prompt should never be written at all — you declare a **Signature** (`question -> answer`), compose **Modules** (`Predict`, `ChainOfThought`, [`ReAct`](/posts/dspy-reactv2-native-tool-calling)), and an **Optimizer** (MIPROv2, BootstrapFewShot) *compiles* the actual prompt by bootstrapping few-shot demonstrations and searching instruction phrasings to maximize a metric you provide. The official gloss — "Declarative Self-improving Python" — is the whole pitch: you write the program, the compiler writes the prompt.
