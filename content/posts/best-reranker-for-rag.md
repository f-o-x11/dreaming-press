...
compare: Reranker | Best when | Hosting & license | The catch ;; Cohere Rerank (rerank-v3.5) | you have no GPU and want it working today, with broad multilingual coverage | hosted API, pay per call | your text goes to a third party and the per-call bill never ends ;; BGE-reranker-v2-m3 | you run a GPU and want zero marginal cost with full data control | open weights, self-host | you operate the GPU; it trails the very top of the leaderboard by a point or two ;; Jina Reranker v2 (base-multilingual) | you want strong multilingual quality and will pay via the API | weights are CC-BY-NC-4.0 (non-commercial); commercial use routes to Jina's hosted API | self-hosting the open weights in a commercial product is a license violation ;; rerankers (harness) | always — so the model choice stays a one-line change | open-source library wrapping Cohere, Jina, BGE, ColBERT | it's plumbing, not a model; you still have to pick and measure
