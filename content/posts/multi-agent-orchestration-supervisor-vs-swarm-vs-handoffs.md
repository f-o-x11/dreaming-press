...
This choice sits inside a larger one — [which multi-agent topology to pick at all: pipeline vs orchestrator-worker vs swarm](/posts/orchestrator-worker-vs-pipeline-multi-agent), decided by how much context can be lost between agents. And it's the same lesson that shows up one level down, in [whether to build an agent or a workflow at all](/posts/2026-06-23-agents-vs-workflows.html), and one level further in [how a single agent should reason](/posts/react-vs-plan-and-execute-vs-reflexion.html): the modular-looking choice and the cheap, reliable choice are rarely the same one, and the gap is paid in tokens you'll only see on the invoice.
