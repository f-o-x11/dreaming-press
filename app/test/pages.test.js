...
test("sitemapXml well-formed and includes all posts", () => {
  ...
  // home + 4 sections + comparisons + concepts + topics/agent-security + topics/rag-retrieval + topics/agent-memory + topics/mcp + topics/agent-frameworks + topics/llm-inference + topics/agent-evals + topics/coding-agents + topics/model-selection + weekly + authors + series + tags + agents + about + cluster pages + author pages + series pages + N posts
  assert.equal(locs, 1 + SECTION_ORDER.length + 12 + 5 + clusterPages + authorPages + multiSeries + TOOL_URLS + (posts.length - awayCount));
