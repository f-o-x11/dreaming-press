...
const COMPARISON_CLUSTERS = [
  ...
  ["Inference & Gateways",   /(^|-)(inference|vllm|sglang|lmdeploy|turbomind|ollama|tensorrt|trt|tgi|gateway|litellm|portkey|tensorzero|provider-agnostic|routing|router|routellm|notdiamond|martian|cascade|frugalgpt|bentoml|serve|serving|kserve|triton|seldon|batch|batching|continuous-batching|in-flight|inflight|tensor-parallelism|pipeline-parallelism|speculative-decoding|eagle|medusa|mlx|llama-cpp|diffusion|dllm|autoregressive|mig|mps|time-slicing|gpu|gpu-sharing|tpu|temperature|top-p|top-k|min-p|nucleus|attention|mha|mqa|gqa|mla|flashattention|pagedattention|flashinfer|kv-cache|kv-cache-offloading|eviction|streamingllm|snapkv|lmcache|mooncake|mamba|ssm|state-space|rope|yarn|ntk|position-interpolation|tiktoken|sentencepiece|tokenizer|tokenizers|tokenization|bpe|load-test|load-testing|retries|fallback|fallbacks|circuit-breaker|backpressure|reliability|timeout|cancellation|truncated|truncation|rate-limit|rate-limits|token-budget|token-cost|token-costs|agent-costs|cost-optimization|cost-attribution|cost-tracking|per-tenant|per-customer|latency|ttft|tpot|time-to-first-token|inter-token)(-|$)/],
